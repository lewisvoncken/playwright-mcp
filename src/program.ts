...
program
    ...
    .option('--proxy-bypass <bypass>', 'comma-separated list of domains to bypass proxy for. Example: "localhost,example.com"')
    .option('--proxy-server <proxy>', 'proxy server to use. Example: "http://localhost:8080"')
    .option('--save-trace', 'save the Playwright trace of the session into the output directory.')
    .option('--storage-state <path>', 'path to the storage state file.')
    .option('--user-agent <agent>', 'user agent to use')
    .option('--user-data-dir <path>', 'path to the user data directory.')
    .option('--video-mode <mode>', 'video recording mode. Can be "off", "on", or "retain-on-failure". Default is "off".')
    .option('--video-responses <mode>', 'whether to send video responses to the client. Can be "allow", "omit", or "auto". Defaults to "auto", which sends videos if the client can display them.')
    .option('--video-size <size>', 'video size in pixels. Example: "1920,1080". Default is "1280,720".')
    .option('--viewport-size <size>', 'viewport size in pixels. Example: "800,600"')
